# Learning docs — read in order

Numbered so they read as a sequence (now and later). Convention: next doc takes the next number; dates live inside each doc.
...
17. 17-self-iteration-loop.md — the capability-request miner (Phase A): friction in v2_turns becomes structured gap requests, provenance machine-set so trust can't be forged (error vs complaint), the /gaps read path, the parity gate that refuses to run until the writer landed
18. 18-timer-tool.md — the timer tool: HA's native assist-timer intent (HassStartTimer/HassCancelTimer) on the ORIGINATING device so the LED wheel spins; device_id via RunnableConfig injection, "Done:" silent-success alignment, honest text/DM degrade — the gap she flagged out loud, closed
