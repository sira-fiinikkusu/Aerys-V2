...
| `MODEL_BACKEND` | `api` (metered) or `oauth` (the Max subscription via the Claude Agent SDK: a warm CLI process, a FRESH session per turn, the full prompt each turn). With `OAUTH_TOOL_BACKEND=oauth` the action path rides the subscription too: tools bind natively, the CLI's PreToolUse hook defers every call, our ToolNode executes it, results ride the next prompt. `OAUTH_VOICE_BACKEND=api` (default) keeps voice metered until the warm-client soak passes. `cli` (langchain-claude-cli) is blocked: its session resume carried a previous turn's context forward. Quota, not dollars: the plan's normal limits, shared with any Claude Code session on the same login; worst case is waiting for a reset, never a bill. The API key stays required for evals, CI and fallback. |
