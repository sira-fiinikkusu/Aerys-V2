...
**Current focus:** Phase 1 — Orchestration Skeleton (01-02 COMPLETE 2026-07-02; next: 01-03 per revised Wave-1 order in planning-repo `.planning/aerys-v2-dossier/CROSS-REVIEW.md`)

## Current Position

Phase: 1 of 12 (Orchestration Skeleton)
Plan: 01-02 COMPLETE (`113c03d`) — factory.py (build_model + START→chat→END graph, checkpointer INJECTED: InMemorySaver now / NAS Postgres Phase 2 per owner decision 7/2), service.py ask() seam with rails (recursion_limit fuse, wall-clock budget, empty-input gate), CLI --ask path, 8 new offline tests (17 green). BUILD RHYTHM CHANGED 7/2: Kael builds during the Fable window, Chris gates at phase boundaries + learns via docs/learning/ (see 2026-07-02-factory-service-ask.md). Dossier + cross-review live in the PRIVATE planning repo.
...
Progress: [████░░░░░░] ~40% (01-01 done + 01-02 state/accessor done; build_model/graph/ask remain)
